...
def get_flashcard_controller_API(db_name:str, type:str='flashcards') -> Union[utils.FlashcardController, int]:
    ''' checks if db file exist, if not returns ERR_CODE else returns inst:FlashcardController'''
    file_path:Path = DB_DIR_PATH / f"{db_name}_flashcards_db.json" if type == 'flashcards' else DB_DIR_PATH / f"{db_name}_keywords_db.json"
...
def add_to(db_name:str) -> Union[str, int]:
    ''' Interface for inputting user provided flashcard qstns and ans '''
...
def add_kws_to(db_name:str) -> Union[str, int]: 
    ''' Interface for inputting user provided keywords '''
    controller_API:Union[utils.FlashcardController, int] = get_flashcard_controller_API(db_name, 'keyword')

    if controller_API == FILE_ERR_CODE_2: 
        return FILE_ERR_CODE_2 
    
    click.echo('\nPlease enter flashcard ID that you would like to add keywords to?')
    click.echo('To cancel type "e"')
    _id = input('>>Flashcard ID: ')

    if _id == 'e':
        return click.echo('\nCancel success\n')
    
    kw_input = input('\nPlease input ' + click.style('Keyword::>> ', fg='cyan'))
    _, COMM_CODE = controller_API.add_kw(kw_input, _id)
...
def get_keywords_from(db_name) -> Dict[str, List[str]]: 
    '''  '''
    controller_API:Union[utils.FlashcardController, int] = get_flashcard_controller_API(db_name, 'keyword')
    if controller_API == FILE_ERR_CODE_2:
        return FILE_ERR_CODE_2
    else: 
        return controller_API.retrieve_dataset('keywords_list')
...
def _random_select_from(dataset, type='flashcards') -> Tuple[int,Tuple[str]]:
    '''  '''
    if type == 'flashcards': 
        selected_key:int = random.choice(list(dataset))
        return (selected_key, dataset[selected_key])
    else: 
        selected_key:str = random.choice(list(dataset.keys()))
        return (selected_key, dataset[selected_key])
...
def begin_study_session(db_name, flashcards_dataset) -> None:
    ...
    while len(flashcards_dataset) != 0:
        flashcard_selected:Tuple[int,Tuple[str]] = _random_select_from(flashcards_dataset)
        click.echo(
...
def begin_teaching_session(db_name, flashcards_dataset, kws_dataset:Dict[str, List[str]]) -> None: 
    '''  '''
    click.echo(click.style(f"\n[NOW TEACHING {db_name.upper()}]:\n", fg="bright_white"))
    click.echo('You can exit at any time, simply enter `e` when prompted an input')

    while len(kws_dataset) != 0:
        kws_selected:Tuple[int, List[str]] = _random_select_from(kws_dataset, 'keywords')
        kws_ID:int = kws_selected[0]
        associated_kws_list:List[str] = kws_selected[1]
        score_point:int = 0 

        click.echo(
            click.style("\n[TEACH ME]:\n", fg='magenta', bold=True) + 
            f'\t{format(flashcards_dataset[kws_ID][0])}\n' #NOTE use namedtuple 
        )
        ...
        for word in ui_ans.split(' '): 
            if word in associated_kws_list: 
                score_point += 1

        if score_point / len(associated_kws_list) >= 0.9: 
        
            click.echo(
                click.style("\n[Result]:\n", fg='bright_blue') + 
                click.style("\tCorrect! Your Answer score is ::>> ", fg='cyan') + f'{score_point / len(associated_kws_list)}\n'  
            )
        else: 
            click.echo(
                click.style("\n[Result]:\n", fg='bright_blue') + 
                click.style("\tWrong! Your Answer is blow the 90% threshold ::>> ", fg='cyan') + f'{ui_ans}\n' + 
                click.style("\tCorrect Answer::>> ", fg='cyan') + f'{format(flashcards_dataset[kws_ID][1])}\n' 
            )

        is_succ_attempt = input('Did you score more than 90%?\nSuccess::[y/N]: ').lower()
        if is_succ_attempt == 'y':
            del kws_dataset[kws_ID] #NOTE M.P.O
        elif is_succ_attempt == 'n':
            continue
        elif is_succ_attempt == 'e':
            return 
