...
def addto(db_name:str) -> None:
    ...
    session = ''
    while session != 'N':
        backend_resp:Union[str, int] = cli.add_to(db_name) # interface 4 input flashcards qstn & ans; returns code msg 
        if backend_resp == FILE_ERR_CODE_2:
            return click.echo(f'\nDatabase not found. \nPlease run, "python3 -m flashCLI init {db_name}"\n')
        else:
            click.echo(backend_resp) # echo flashcard was added 
...
def initkws4(db_name) -> None: 
    ''' Init keyword list container 2 contain keywords '''
...
@main.command()
@click.argument('db_name')
def addkws2(db_name) -> None: 
    ''' Provide keywords that will be used in conjunction with the cmd `teach` '''
    session = ''

    while session != 'N': 
        backend_resp:Union[str, int] = cli.add_kws_to(db_name) 

        if backend_resp == FILE_ERR_CODE_2: 
            return click.echo(f'\nDatabase not found. \nPlease run, "python3 -m flashCLI initkws4 {db_name}"\n')
        else: 
            click.echo(backend_resp)

        session = input('\nDo you wish to add more keywords?\n' + click.style('Add More?::', fg='bright_yellow') + '[y/N]: ').upper()

    click.echo(f'\nThank you, your keywords have been successfully added to {db_name}\n')



@main.command()
@click.argument('db_name')
def teachme(db_name) -> None: 
    ''' Prompts user to teach '''
    flashcards_dataset:Union[int, Dict[int, Tuple[str]]] = cli.get_flashcards_from(db_name)
    kws_dataset:Dict[str, List[str]] = cli.get_keywords_from(db_name)
    ...
    cli.begin_teaching_session(db_name, flashcards_dataset, kws_dataset)
    click.echo(click.style('\n[SUCCESS]:: ', fg='bright_white') + 'Study session over\n') 
