...
def _is_in_range_of_target(num:int, trg:int=100, threshold:int=10) -> bool: 
    '''  
        VAR lower_bound = trg - threshold 
        VAR upper_bound = trg + threshold 
        VAR default==_cond = num == 2 * trg 
        
        IF lower_bound <= num <= upper_bound OR default_cond 
            RETURN true 
        RETURN false 
    '''
    lower_bound = trg - threshold 
    upper_bound = trg + threshold 
    default_cond = num == 2 * trg 
...
def _remove_str_at_index(txt:str, idx:int) -> str: 
    '''  
        TRY 
            IF len(txt) == 0 
                RAISE ValueError('string cannot be zero length')
...
def _heptiptup(n:int) -> str: 
    '''  
        ...
        RETURN res IF len(res) > 0 else str(n)
    '''
    res = '' 
    rules = {
        '3': 'Hep', 
...
def _calculate_slope(point_a: Dict[str,float], point_b: Dict[str, float]) -> float: 
    '''  
        TRY 
            IF [
                ...
                RAISE ValueErr 
            VAR delta_y = point_b['y'] - point_a['y']
            VAR delta_x = point_b['x'] - point_a['x']
    ...
    try: 
        if (
            ...
            raise ValueError('preReq missing')
...
def _custom_heptiptup(num: int, rule: Dict[str, int]) -> str:
    '''  
        TRY 
            IF len(rule) != 3 OR any([val for val IN rule.keys()])
                RAISE ValueError 
            VAR res_str = '' 
            itr(rule, txt, val) 
                IF num % val == 0 
                    res_str += txt 
            RETURN res_str if len(res_str) > 0 else str(num) 
        EXCEPT ValueError 
            RETURN repr(err)
    ''' 
    try: 
        ...
        if len(rule) != 3 or any([val < 0 for val in rule.values()]): 
            raise ValueError('preReq not meet')
        
        for txt, val in rule.items(): 
            if num % val == 0: 
                res_str += txt 
