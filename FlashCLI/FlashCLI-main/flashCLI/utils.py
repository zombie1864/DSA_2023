...
class FlashcardController:
    ...
    def add(self, qstn:str, ans:str) -> CurrentFlashCard:
        ...
        if len(db_resp.dataset) == 0: # init it will be {}
            ...
            # sets data; db_resp.dataset[99] = (qstn, ans)
    ...
    def add_kw(self, kw:str, flashcard_id:str) -> Keyword:
        '''  '''
        db_resp:DBResponse = self._db_handler.read_flashcard_dataset('keyword') 
        
        if db_resp.COMM_CODE == DB_READ_ERR_CODE_3:
            return Keyword(kw, db_resp.COMM_CODE)
        
        if len(db_resp.dataset) == 0: 
            print('init branch')
            db_resp.dataset[flashcard_id] = [kw]
        elif db_resp.dataset.get(flashcard_id) is not None: 
            db_resp.dataset[flashcard_id].append(kw)
        elif len(db_resp.dataset) > 0 and db_resp.dataset.get(flashcard_id) is None: 
            print('second branch')
            db_resp.dataset[flashcard_id] = [kw]
        
        db_req:DBResponse = self._db_handler.write_flashcard_dataset(db_resp.dataset, 'keyword')
        
        return Keyword(kw, db_req.COMM_CODE)



    def retrieve_dataset(self, type='flashcards') -> Dict[int, Tuple[str]]:
        ...
        elif type == 'keywords_list': 
            db_resp = self._db_handler.read_flashcard_dataset('keywords_list')
            return db_resp.dataset
